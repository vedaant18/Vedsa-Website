...
function Particles({ scrollSpeed }: { scrollSpeed: number }) {
  ...
  useFrame((state) => {
    if (groupRef.current) {
      const speed = 0.02 + scrollSpeed * 0.08;
...
function FloatingRings({ scrollSpeed }: { scrollSpeed: number }) {
  ...
  useFrame((state) => {
    ...
    const speed = 0.3 + scrollSpeed * 0.4;
    if (ring1Ref.current) {
...
function WaveGrid({ scrollSpeed }: { scrollSpeed: number }) {
  ...
  useFrame((state) => {
    if (gridRef.current) {
      const time = state.clock.elapsedTime * (0.3 + scrollSpeed * 0.4);
...
export default function AnimatedBackground() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      scrollVelocity = Math.abs(currentScrollY - lastScrollY) / 200;
      lastScrollY = currentScrollY;
      setScrollSpeed(scrollVelocity);
      
      setTimeout(() => {
        setScrollSpeed(prev => prev * 0.96);
      }, 180);
    };

...
function Particles({ scrollSpeed }: { scrollSpeed: number }) {
  ...
  useFrame((state) => {
    if (groupRef.current) {
      const speed = 0.02 + scrollSpeed * 0.01;
...
function FloatingRings({ scrollSpeed }: { scrollSpeed: number }) {
  ...
  useFrame((state) => {
    ...
    const speed = 0.3 + scrollSpeed * 0.05;
    if (ring1Ref.current) {
...
function WaveGrid({ scrollSpeed }: { scrollSpeed: number }) {
  ...
  useFrame((state) => {
    if (gridRef.current) {
      const time = state.clock.elapsedTime * (0.3 + scrollSpeed * 0.05);
...
export default function AnimatedBackground() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      scrollVelocity = Math.abs(currentScrollY - lastScrollY) / 500;
      lastScrollY = currentScrollY;
      setScrollSpeed(scrollVelocity);
      
      setTimeout(() => {
        setScrollSpeed(prev => prev * 0.98);
      }, 200);
    };
